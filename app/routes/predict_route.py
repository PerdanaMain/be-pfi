...
@token_required
@app.route(prefix + "/predict-detail", methods=["GET"])
def predict_detail():
    return detail()
...
def stream_values():
    ...
    try:
        ...
        sleep = request.args.get("sleep", default=1, type=int)
        return Response(
            stream_tag_value(tag_id, size, sleep), content_type="text/event-stream"
        )
...
def stream_prediction_values():
    ...
    try:
        ...
        sleep = request.args.get("sleep", default=1, type=int)
        return Response(
            stream_tag_prediction_value(tag_id, size, sleep),
            content_type="text/event-stream",
        )
    except Exception as e:
        # Format error response
